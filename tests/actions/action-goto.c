...
START_TEST(test_action_goto_new) {
} END_TEST

START_TEST(test_action_goto_get_type) {
  zathura_action_type_t type;
  fail_unless(zathura_action_get_type(action, &type) == ZATHURA_ERROR_OK);
  fail_unless(type == ZATHURA_ACTION_GOTO);
} END_TEST

START_TEST(test_action_goto_init) {
  /* invalid arguments */
  fail_unless(zathura_action_goto_init(NULL)
  ...
  fail_unless(zathura_action_goto_init(action_unknown) == ZATHURA_ERROR_ACTION_INVALID_TYPE);
...
START_TEST(test_action_goto_clear) {
  /* invalid arguments */
  fail_unless(zathura_action_goto_clear(NULL)
      == ZATHURA_ERROR_INVALID_ARGUMENTS);

  zathura_action_t* action_unknown;
  fail_unless(zathura_action_new(&action_unknown, ZATHURA_ACTION_UNKNOWN) == ZATHURA_ERROR_OK);
  fail_unless(action_unknown != NULL);
  fail_unless(zathura_action_goto_clear(action_unknown) == ZATHURA_ERROR_ACTION_INVALID_TYPE);
  fail_unless(zathura_action_free(action_unknown) == ZATHURA_ERROR_OK);

  /* valid arguments */
  fail_unless(zathura_action_goto_clear(action)
      == ZATHURA_ERROR_OK);
} END_TEST