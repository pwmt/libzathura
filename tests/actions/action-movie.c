...
START_TEST(test_action_movie_init) {
  ...
  fail_unless(zathura_action_movie_init(action_unknown) == ZATHURA_ERROR_ACTION_INVALID_TYPE);
...
START_TEST(test_action_movie_clear) {
  ...
  fail_unless(zathura_action_new(&action_unknown, ZATHURA_ACTION_UNKNOWN) == ZATHURA_ERROR_OK);
  fail_unless(action_unknown != NULL);
  fail_unless(zathura_action_movie_clear(action_unknown) == ZATHURA_ERROR_ACTION_INVALID_TYPE);
  fail_unless(zathura_action_free(action_unknown) == ZATHURA_ERROR_OK);
